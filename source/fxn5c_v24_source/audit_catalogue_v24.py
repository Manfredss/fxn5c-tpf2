...
def audit_model_localization(target,strings):
    """Exercise real _() calls, not the identity shim used for structural checks."""
    expected={r['stem']:description_key(r) for r in ROSTER}
    expected.update(GROUPS)
    rows=[];negative_controls=0
    for stem,key in sorted(expected.items()):
        path=target/'res/models/model/vehicle/train'/(stem+'.mdl')
        source=path.read_text(encoding='utf-8')
        keys=(key+'_NAME',key+'_DESC')
        for language,translations in strings.items():
            def evaluate(text):
                ...
                def translate(s):
                    calls.append(s)
                    assert s in translations,('missing runtime translation',stem,language,s)
                    return translations[s]
                vm.globals()['_']=translate
                vm.execute(text)
                model=plain(vm.globals().data())
                actual=model['metadata']['description']
                want={'name':translations[keys[0]],'description':translations[keys[1]]}
                assert actual==want,('untranslated MDL description',stem,language,actual,want)
                assert sorted(calls)==sorted(keys),('MDL must execute both description translations',stem,language,calls)
                return actual,calls
            actual,calls=evaluate(source)
            # Mutate only a transient string: native files remain untouched.
            missing_calls=source
            for token in keys:
                pattern=r'_\s*\(\s*"'+re.escape(token)+r'"\s*\)'
                missing_calls,count=re.subn(pattern,'"'+token+'"',missing_calls)
                assert count==1,('expected one localized MDL field',stem,token,count)
            try:evaluate(missing_calls)
            except AssertionError:negative_controls+=1
            else:raise AssertionError(('identity-only translation regression was not detected',stem,language))
            rows.append({'stem':stem,'language':language,'resolved_description':actual,
                         'translation_calls':calls,'identity_only_negative_control':'DETECTED'})
    assert len(rows)==24 and negative_controls==24
    return {'localized_model_language_cases':len(rows),'localized_description_fields':len(rows)*2,
            'identity_only_negative_controls':negative_controls,'models':rows}
...
def main():
    ...
    if args.fixtures_only:
        with tempfile.TemporaryDirectory(prefix='fxn5c_catalogue24_') as folder:
            target=write_catalogue(Path(folder))
            for name in ('mod.lua','strings.lua'):
                p=target/name
                p.write_text(p.read_text(encoding='utf-8').replace('v0.21','v0.24').replace('minorVersion = 21','minorVersion = 24'),encoding='utf-8')
    ...
    assert info['lua_fixture_cases']==555, info
    rows=audit_models(MOD_ROOT,strings)
    localization=audit_model_localization(MOD_ROOT,strings)
    inputs=[ROOT/'audit_catalogue_v24.py',ROOT/'catalogue_v21.py',ROOT/'roster_v21.py',MOD_ROOT/'mod.lua',MOD_ROOT/'strings.lua']+sorted((MOD_ROOT/'res/models/model/vehicle/train').glob('*.mdl'))
    report={'status':'PASS','scope':'Lua definitions, translations, exact-ID modifier and native catalogue; NOT game UI test',
            'game_ui_verified':False,**info,'vehicles':rows,'groups':GROUPS,'runtime_localization':localization,
            'inputs_sha256':{p.relative_to(ROOT).as_posix():hashlib.sha256(p.read_bytes()).hexdigest() for p in inputs}}
    out=ROOT/'catalogue_audit_v24.json';out.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')

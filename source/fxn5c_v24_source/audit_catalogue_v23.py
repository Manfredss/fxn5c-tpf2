...
def main():
    ...
    if args.fixtures_only:
        with tempfile.TemporaryDirectory(prefix='fxn5c_catalogue21_') as folder:
            ...
            for name in ('mod.lua','strings.lua'):
                p=target/name
                p.write_text(p.read_text(encoding='utf-8').replace('v0.21','v0.22').replace('minorVersion = 21','minorVersion = 22'),encoding='utf-8')
    ...
    out=ROOT/'catalogue_audit_v23.json';out.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')

"""Encode and independently decode actual v24 native-readback icon masters.

Preserves native resource names and descriptor 0 bottom-origin BGRA layout.
No inherited v21/v22/v23 image is accepted as a substitute for a current render.
"""
...
def render_evidence():
    ...
    for style in ('cr','jinwen'):
        candidates=[ROOT/f'render_audit_v24_{style}_{mode}.json' for mode in ('all','icons')]
        ...
        for row in report['icons']:
            ...
            assert lights['direction']=='fwd' and lights['position']=='singleton'
            assert len(lights['visible_names'])==1 and lights['engine_tested'] is False
...
def main():
    ...
    report['status']='PASS'
    (ROOT/'ui_icon_audit_v24.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
    print('48 v24 bottom-origin icons PASS',flush=True)
